...
async function fetchEligibleDrivers(req, res) {
  const todayFrench = getFrenchDayName(); // جلب اليوم الحالي باللغة الفرنسية
  let connection;

  try {
    connection = await pool.getConnection();
    console.log(`🔍 اليوم الحالي باللغة الفرنسية: ${todayFrench}`);

    // ✅ 1️⃣ استعلام لجلب جميع المرشحين المؤهلين بدون JSON_CONTAINS()
...
// 🔹 تعريف المسار
router.get('/eligible-drivers', fetchEligibleDrivers);
