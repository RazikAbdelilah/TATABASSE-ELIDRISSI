...
router.get('/generate-pdf', async (req, res) => {
    const { schoolName, categorieDomandee } = req.query;

    if (!schoolName || !categorieDomandee) {
        return res.status(400).json({ success: false, message: 'School name and category are required' });
    }
    ...
    try {
        ...
        // استعلام لإحضار البيانات حسب المدرسة والفئة والتاريخ
        const [drivers] = await connection.execute(`
            SELECT monitor, nome, prenom, cin 
            FROM List_of_drivers
            WHERE nome_school = ? 
            AND categorie_domandee = ? 
            AND DATE(date) = ?
        `, [schoolName, categorieDomandee, formattedDate]);

        if (drivers.length === 0) {
            return res.status(404).json({ success: false, message: 'No data found for this school and category on this date.' });
        }
        ...
        drivers.forEach(driver => {
            ...
            monitors[driver.monitor].push(`${driver.nome} ${driver.prenom}  ${driver.cin}`);
        });

        const doc = new PDFDocument({ margin: 30, size: 'A4', layout: 'portrait' });
        const filePath = `./${schoolName}_drivers_${categorieDomandee}.pdf`;
        ...
        // عنوان الـ PDF مع إضافة الفئة
        doc.fontSize(18).text(`Journée des conducteurs ${schoolName} - ${categorieDomandee} ${formattedDate}`, { align: 'center' });
        doc.moveDown(1);

        // إعداد الجدول
        const tableTop = doc.y + 20;
        const columnWidth = 170; // عرض العمود
        const rowHeight = 20;
        const margin = 40;
